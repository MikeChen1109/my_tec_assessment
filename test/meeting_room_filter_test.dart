...
void main() {
  test('refreshForFilter applies centre and capacity filters', () async {
    final cityA = _buildCity(code: 'CITY_A', name: 'City A');
    final cityB = _buildCity(code: 'CITY_B', name: 'City B');
    final centres = [
      _buildCentre(
        id: 'A1',
        cityCode: cityA.code,
    ...
    final pricing = [
      ...
      _buildPricing(roomCode: 'R2', price: 200),
      _buildPricing(roomCode: 'R3', price: 300),
      _buildPricing(roomCode: 'R4', price: 400),
    ];

    var filterState = MeetingRoomFilterState.initial();
    ...
    filterState = filterState.copyWith(
      capacity: 5,
      selectedCentreCodes: const ['A1'],
      centresLabel: 'All Centres in City A',
    );

    await notifier.refreshForFilter();

    final filteredRooms = notifier.state.roomsWithPricing;
    expect(filteredRooms.length, 1);
    expect(filteredRooms.single.room.roomCode, 'R2');
  });
}
